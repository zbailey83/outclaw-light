...
const frameCount = 192; // Total frames for video 2
const currentFrame = index => `frames2/frame_${index.toString().padStart(3, '0')}.webp`;

const images = [];
const airplay = { frame: 1 };
let timelineInitialized = false;

// Set dimensions to match original resolution (1280x720 landscape)
canvas.width = 1280;
canvas.height = 720;
...
function initInteractiveHero() {
  ...
  console.log("Timeline Gen 2 initializing. Frame count:", frameCount);

  // Pinned Master timeline across 7500px scroll travel (extra depth for 192 frames)
  ...
  // Smooth floating point frame tween (Math.floor is executed inside render)
  tl.to(airplay, {
    frame: frameCount,
    ease: "none",
    duration: 10,
  ...
  // Card 2 Animation (Middle Turnaround spin climax: timeline 3.7 – 6.3, frames 71 - 120)
  // Activates right in the middle as the car turns around, featuring 1st Etsy CTA
  tl.fromTo(".card-2", 
  ...
  // Card 3 Animation (End frame climax: timeline 5.8 – 10, frames 112 - 192)
  // Reveals at second half of frames, positioned in bottom right to cover the watermark
  tl.fromTo(".card-3", 
    { autoAlpha: 0, y: 40 }, 
...
for (let i = 1; i <= frameCount; i++) {
  ...
  img.onload = () => {
    ...
    if (loaderLog) {
      loaderLog.textContent = `CACHED // frames2/frame_${i.toString().padStart(3, '0')}.webp`;
    }
