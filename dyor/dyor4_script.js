...
const frameCount = 192; // Total frames for video 4
const currentFrame = index => `frames4/frame_${index.toString().padStart(3, '0')}.webp`;
...
function autoScrollForward() {
  ...
  autoScrollTween = gsap.to(scrollObj, {
    y: maxScroll,
    duration: 7, // 7 seconds smooth forward scrub
    ease: "power1.inOut",
    onUpdate: () => {
...
function autoScrollBackward() {
  ...
  autoScrollTween = gsap.to(scrollObj, {
    y: 0,
    duration: 7, // 7 seconds smooth rewind scrub
    ease: "power1.inOut",
    onUpdate: () => {
      window.scrollTo(0, scrollObj.y);
    }
...
// Event listeners for auto-scroll buttons
if (btnForward) {
  btnForward.addEventListener("click", autoScrollForward);
}
if (btnBackward) {
  btnBackward.addEventListener("click", autoScrollBackward);
}
...
function render() {
  ...
  // Toggle button availability based on exact frame index bounds
  if (frameIndex <= 5) {
    if (btnForward) btnForward.classList.add("active");
    if (btnBackward) btnBackward.classList.remove("active");
  } else if (frameIndex >= 188) { // Final 5 frames (188, 189, 190, 191, 192)
...
function initInteractiveHero() {
  ...
  console.log("Timeline Variation 4 initialized. Frame count:", frameCount);
  ...
  // Card 2 Animation (Three Billboards scene: timeline 3.7 – 6.3)
  tl.fromTo(".card-2", 
